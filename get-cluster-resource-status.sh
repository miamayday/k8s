...
do
  ...
  unreserved_cpu_base="$(grep 'Unreserved:' status.tmp | awk '{print $2}' | grep -oE '^(\.|[0-9])+')"
  unreserved_mem_gi="$(grep 'Unreserved:' status.tmp | awk '{print $4}' | grep -oE '^(\.|[0-9])+')"
  ...
  printf '%s %.1f %.2fGi %.1f %.2fGi %.1f %.2fGi %.1f %.2fGi\n' "${node}" "${used_cpu_base}" "${used_mem_gi}" "${unused_cpu_base}" "${unused_mem_gi}" "${reserved_cpu_base}" "${reserved_mem_gi}" "${unreserved_cpu_base}" "${unreserved_mem_gi}"
done < nodes.tmp

printf 'Total %.1f %.2fGi %.1f %.2fGi %.1f %.2fGi %.1f %.2fGi\n' "${total_used_cpu_base}" "${total_used_mem_gi}" "${total_unused_cpu_base}" "${total_unused_mem_gi}" "${total_reserved_cpu_base}" "${total_reserved_mem_gi}" "${total_unreserved_cpu_base}" "${total_unreserved_mem_gi}"
